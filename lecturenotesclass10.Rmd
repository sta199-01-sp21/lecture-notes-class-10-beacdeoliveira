...
84 patients were enrolled in the clinical trail.
...
```{r simulate_draws}
draw_results <- sample(marbles, size = 1000000, replace = TRUE)
```
...
This is very close to the "true" probability. Almost identical.
...
It depends on the color of the marble. I'd roughly guess 35%.
...
You add the variable using the "mutate" command. 
...
The square of dice roll 1 is equal to dice roll 2.
...
```{r simulate_rolls}
roll_results <- tibble(
  die_1 = replicate(n = 10000, expr = sample(1:6, size =1)),
  die_2 = replicate(n = 10000, expr = sample(1:6, size = 1)
))
```

```{r identify_events}
roll_results <- roll_results %>% 
  mutate(number_match = die_1 ^ 2 == die_2,
         abs_value = abs(die_1 - die_2) == 4)
roll_results
```

```{r summarize_rolls}
roll_results %>% 
  summarise(event_1_prop = mean(number_match),
            event_2_prop = mean(abs_value))
```
